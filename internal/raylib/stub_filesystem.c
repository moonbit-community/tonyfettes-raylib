...
moonbit_raylib_load_dropped_files(void) {
  return MakeFilePathListWrapper(LoadDroppedFiles(), 1);
}
...
moonbit_raylib_load_directory_files(moonbit_bytes_t dirPath) {
  return MakeFilePathListWrapper(LoadDirectoryFiles((const char *)dirPath), 0);
}

FilePathListWrapper *
moonbit_raylib_load_directory_files_ex(moonbit_bytes_t basePath, moonbit_bytes_t filter, int scanSubdirs) {
  return MakeFilePathListWrapper(LoadDirectoryFilesEx((const char *)basePath, (const char *)filter, (bool)scanSubdirs), 0);
}
